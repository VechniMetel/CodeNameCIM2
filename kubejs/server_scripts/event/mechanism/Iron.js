...
BlockEvents.rightClicked("ue_addons:the_accelerator_of_mechanism_power", (event) => {
	...
	if (event.getItem().is("ue_addons:iron_mechanism")) {
		...
		for (let m = 0; m <= 4; m++) {
			...
			for (let n = 0; n <= 4; n++) {
				...
				if (block === "minecraft:stone") {
					rand = Math.random()
					if (rand <= 0.2) {
						event.level.getBlock(pos).set("minecraft:iron_ore")
					} else if (rand <= 0.25) {
						event.level.getBlock(pos).set("minecraft:redstone_ore")
					} else if (rand <= 0.3) {
						event.level.getBlock(pos).set("thermal:nickel_ore")
					}
				} else if (block === "minecraft:deepslate") {
					rand = Math.random()
					if (rand <= 0.15) {
						...
						event.level.getBlock(pos).set("thermal:deepslate_nickel_ore")
					}
				}
			}
		}
...
BlockEvents.rightClicked((event) => {
	...
	if (item.id !== "ue_addons:iron_mechanism" || block.id !== "ue_addons:the_accelerator_of_mechanism_power") {
		return
	}
	let command = `particle minecraft:dust 1 1 1 1 ${block.x + 0.5} ${block.y + 1} ${block.z + 0.5} 0.5 0.5 0.5 0.1 30`
