BlockEvents.rightClicked("ue_addons:the_accelerator_of_mechanism_power", r => {
	...
	if (r.getItem().is('ue_addons:enchanted_mechanism')) {
		...
		for (let m = 0; m <= 4; m++) {
			...
			for (let n = 0; n <= 4; n++) {
				...
				if (block === 'minecraft:stone') {
					rand = Math.random()
					if (rand <= 0.2) {
						r.level.getBlock(pos).set('minecraft:lapis_ore')
					}else if (rand <= 0.21) {
						r.level.getBlock(pos).set('create:experience_block')
					}
				}
				else if (block === 'minecraft:deepslate') {
					rand = Math.random();
					if (rand <= 0.2) {
						r.level.getBlock(pos).set('minecraft:deepslate_lapis_ore')
					}else if (rand <= 0.21) {
						r.level.getBlock(pos).set('create:experience_block')
					}
...
BlockEvents.rightClicked(r => {
	...
	if (item.id !== 'ue_addons:enchanted_mechanism' || block.id !== 'ue_addons:the_accelerator_of_mechanism_power') return;
