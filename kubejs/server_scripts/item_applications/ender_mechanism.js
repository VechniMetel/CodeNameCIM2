...
ItemEvents.rightClicked('ue_addons:ender_mechanism', event => {
    ...
    // 3. 定义传送范围（5x5 区域）
    const range = 2; // 5x5 区域的范围半径
    const centerX = player.x;
    const centerZ = player.z;
    const yLevel = player.y;

    // 4. 生成随机偏移量
    const randomX = centerX + Math.floor(Math.random() * (range * 2 + 1)) - range;
    const randomZ = centerZ + Math.floor(Math.random() * (range * 2 + 1)) - range;

    // 5. 获取目标位置（确保 Y 坐标在地面上）
    const targetPos = { x: randomX, y: yLevel, z: randomZ };
