Ponder.registry((event) => {
    event.create([
    ...
    ]).scene("usage", "Crucible Usage", "cmi:multiblock/crucible_usage", (scene) => {
        scene.showBasePlate()
        scene.idle(20)
        ...
        // 你可以使用GUI或使用自动输入
        scene.text(20, "You can either use GUI or automation input", [1, 1, 2])
        scene.idle(40)
        
        scene.addKeyframe()
        
		// 配料可以扔进坩埚
        scene.text(20, "Ingredients can be throw into crucible", [2, 5, 3])
        scene.idle(20)
        ...
        scene.idle(20)
        scene.showControls(10, [2, 5, 3], "up")
            .withItem("thermal:iron_dust")
        scene.idle(12)
        scene.world.createItemEntity([2.5, 5, 3.5], Direction.DOWN, "thermal:iron_dust")
        scene.idle(40)
